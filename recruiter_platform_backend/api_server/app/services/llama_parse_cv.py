"""LlamaCloud parsing API (HTTP): markdown for embeddings + structured JSON for CV profile.

Avoids the ``llama_parse`` / ``llama_cloud`` Python packages, which can ship with a broken
submodule layout on some platforms (import-time ``ModuleNotFoundError``).
"""
...
def _debug_log(message: str, *, hypothesis_id: str, data: dict[str, Any]) -> None:
    # #region agent log
    try:
        log_path = Path(__file__).resolve().parents[4] / "debug-2354bc.log"
        payload = {
            "sessionId": "2354bc",
            "hypothesisId": hypothesis_id,
            "location": "llama_parse_cv.py",
            "message": message,
            "data": data,
...
def _structured_payload_from_json_layout(layout: dict[str, Any]) -> dict[str, Any] | None:
    """Pull schema-filled objects from LlamaParse ``pages[].structuredData`` (see JobResult in SDK)."""
...
async def _parse_pdf_http(
    *,
    ...
    # #region agent log
    _debug_log(
        "httpx_llamacloud_parse_start",
        hypothesis_id="H1",
        data={
            "base_host": base.split("//", 1)[-1][:80] if base else "",
            "file_name_suffix": Path(file_name).suffix,
            "fetch_result_type": fetch_result_type,
            "return_json_layout": return_json_layout,
            "form_keys": sorted(form_fields.keys()),
        },
    )
    # #endregion
    ...
    async with httpx.AsyncClient(
        ...
        # #region agent log
        _debug_log(
            "httpx_llamacloud_job_created",
            hypothesis_id="H2",
            data={"job_id_prefix": str(job_id)[:12]},
        )
        # #endregion
        ...
        if return_json_layout:
            n_pages = len(result_json.get("pages") or [])
            # #region agent log
            _debug_log(
                "httpx_llamacloud_parse_done_json_layout",
                hypothesis_id="H_json",
                data={
                    "pages": n_pages,
                    "has_structured": _structured_payload_from_json_layout(result_json) is not None,
                },
            )
            # #endregion
            return result_json

        text = _text_from_result_payload(result_json, fetch_result_type)

        # #region agent log
        _debug_log(
            "httpx_llamacloud_parse_done",
            hypothesis_id="H2",
            data={"chars": len(text)},
        )
        # #endregion
...
async def llamaparse_pdf_to_markdown(
    *,
    api_key: str,
    pdf_bytes: bytes,
    file_name: str,
) -> str:
    """LlamaCloud PDF → markdown for chunking and embeddings."""
    form: dict[str, Any] = {
        "from_python_package": True,
        "save_images": True,
        "system_prompt_append": (
            "This document is a job candidate CV or résumé. Preserve headings, "
            "bullet lists, and employment sections clearly in markdown."
        ),
    }
    return await _parse_pdf_http(
        api_key=api_key,
        pdf_bytes=pdf_bytes,
        file_name=file_name,
        form_fields=form,
        fetch_result_type="markdown",
    )
...
async def llamaparse_pdf_to_structured_profile(
    *,
    ...
    """Structured extraction into CvStructuredProfile via LlamaCloud JSON schema."""
    # ``structured_output_json_schema_name`` is only for LlamaParse *built-in* named schemas
    # (see LlamaParse field description: e.g. imFeelingLucky). Sending a custom class name
    # makes the API look it up in their catalog → "Named Schema … not found".
    schema_str = json.dumps(CvStructuredProfile.model_json_schema())
    # #region agent log
    _debug_log(
        "structured_schema_custom_only",
        hypothesis_id="H_schema",
        data={
            "schema_chars": len(schema_str),
            "omit_schema_name": True,
        },
    )
    # #endregion
    form: dict[str, Any] = {
